...
const EXPORTS: ExportItem[] = [
  {
    ...
    select: 'sku, barcode, color, size, stock_quantity, low_stock_threshold, price_override, product:products(name, base_price, cost_price, sku_prefix)',
    filename: 'สินค้า.csv',
    transform: (rows) => rows.map((r: any) => ({
  ...
  },
  {
    ...
    icon: '👥',
    table: 'customers',
    select: 'full_name, phone, email, address, total_spent, visit_count, loyalty_points, tier, created_at',
    filename: 'ลูกค้า.csv',
    transform: (rows) => rows.map((r: any) => ({
  ...
  },
  {
    ...
    icon: '🧾',
    table: 'sales',
    select: 'sale_number, total, subtotal, discount_amount, payment_method, status, created_at, cashier:profiles!cashier_id(full_name)',
    filename: 'การขาย.csv',
    transform: (rows) => rows.map((r: any) => ({
  ...
  },
  {
    ...
    icon: '🪡',
    table: 'job_orders',
    select: 'order_number, customer_name, customer_phone, description, status, quoted_price, deposit_amount, due_amount, estimated_completion_date, created_at',
    filename: 'งานปัก.csv',
    transform: (rows) => rows.map((r: any) => ({
  ...
  },
  {
    ...
    select: 'plan_number, customer_name, customer_phone, item_description, total_amount, down_payment, balance_amount, total_installments, interval_days, status, created_at',
...
export default function DataExportSettings() {
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      <h3 className="text-lg font-bold text-gray-800 mb-1">ส่งออกและสำรองข้อมูล</h3>
      <p className="text-sm text-gray-500 mb-4">ดาวน์โหลดข้อมูลเป็นไฟล์ CSV สำหรับสำรองหรือนำเข้า Excel</p>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {EXPORTS.map(item => (
          ...
          >
            ...
            <span className="text-brand-500 text-sm font-medium flex-shrink-0">⬇️ CSV</span>
          </button>
        ))}
      </div>
    </div>

...
export default function BottomNav() {
  ...
  const primaryTabs = [
    { href: '/dashboard', icon: <LayoutDashboard size={22} />, label: 'หน้าหลัก' },
    { href: '/pos', icon: <ShoppingCart size={22} />, label: 'ขายสินค้า', roles: ['admin', 'manager', 'cashier'] },
    { href: '/job-orders', icon: <Scissors size={22} />, label: 'งานปัก' },
    { href: '/inventory', icon: <Package size={22} />, label: 'สต็อก', roles: ['admin', 'manager'] },
  ].filter(tab => !tab.roles || hasRole(tab.roles))

  // Secondary items — shown in drawer
  const drawerItems = [
    { href: '/risk-dashboard', icon: <BarChart3 size={20} />, label: 'แดชบอร์ดความเสี่ยง', roles: ['admin', 'manager'] },
    { href: '/customers', icon: <Users size={20} />, label: 'ลูกค้า', roles: ['admin', 'manager'] },
    { href: '/reports', icon: <FileText size={20} />, label: 'รายงาน', roles: ['admin', 'manager'] },
    { href: '/settings', icon: <Settings size={20} />, label: 'ตั้งค่า', roles: ['admin'] },
        ...
        <div className="flex items-stretch h-16">
          {primaryTabs.map(tab => (
            <Link
              ...
              `}
            >
              <span className={isActive(tab.href) ? 'text-brand-600' : 'text-gray-400'}>
                {tab.icon}
              </span>

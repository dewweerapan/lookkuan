...
async function getDashboardStats() {
  ...
  const [
    ...
    supabase
      ...
      .select(
        'id, sku, stock_quantity, low_stock_threshold, product:products(name)',
      )
      .eq('is_active', true)
  ...
  const { data: dueSoonJobs } = await supabase
    .from('job_orders')
    .select(
      'id, order_number, customer_name, estimated_completion_date, status',
    )
    .in('status', ['pending', 'in_progress'])
  ...
  const { data: todayInstallments } = await supabase
    ...
    .select(
      'id, installment_number, due_date, amount, plan_id, plan:installment_plans(plan_number, customer_name)',
    )
    .eq('status', 'pending')
    .lte('due_date', today)
  ...
  return (
    <div>
      ...
      <div className='grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8'>
        <Link
          href='/pos'
          className='pos-card bg-brand-50 border-brand-200 hover:border-brand-400'
        >
        ...
        <Link
          href='/job-orders/new'
          className='pos-card bg-blue-50 border-blue-200 hover:border-blue-400'
        >
        ...
        <Link
          href='/inventory'
          className='pos-card bg-green-50 border-green-200 hover:border-green-400'
        >
          <span className='text-3xl'>📦</span>
        ...
        <Link
          href='/customers'
          className='pos-card bg-purple-50 border-purple-200 hover:border-purple-400'
        >
          <span className='text-3xl'>👥</span>
      ...
      {stats.dueSoonJobs.length > 0 && (
        <div className='bg-orange-50 border-2 border-orange-200 rounded-xl p-6 mb-6'>
          ...
          <Link
            href='/job-orders'
            className='inline-block mt-3 text-orange-700 font-semibold hover:text-orange-800 text-sm'
          >
            ดูงานทั้งหมด →
          </Link>
        </div>
      ...
      {stats.todayInstallments.length > 0 && (
        <div className='bg-purple-50 border-2 border-purple-200 rounded-xl p-6 mb-6'>
          ...
          <Link
            href='/installments'
            className='inline-block mt-3 text-purple-700 font-semibold hover:text-purple-800 text-sm'
          >
            ดูผ่อนชำระทั้งหมด →
          </Link>
        </div>
          ...
          <div className='space-y-2'>
            {(stats.lowStockItems as LowStockVariant[]).map((item) => (
              ...
              >
                ...
                <span
                  className={`font-bold ${item.stock_quantity <= 0 ? 'text-red-600' : 'text-yellow-600'}`}
                >

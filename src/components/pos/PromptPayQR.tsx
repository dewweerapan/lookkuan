...
export default function PromptPayQR({ phone, amount, storeName }: Props) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-2">
      {qrDataUrl ? (
        <img src={qrDataUrl} alt="PromptPay QR" className="w-52 h-52 rounded-xl border border-gray-200" />
      ) : (
      ...
      <div className="text-center">
        ...
        {storeName && <p className="text-sm text-gray-600 font-semibold">{storeName}</p>}
        <p className="text-2xl font-bold text-brand-600 mt-1">{formatCurrency(amount)}</p>
      </div>
    </div>
  )
}

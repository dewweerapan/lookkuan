...
export default function StoreLogoSettings() {
  ...
  const handleSave = async (url: string | null) => {
    ...
    try {
      ...
      await supabase
        .from('store_settings')
        .upsert({ key: 'store_logo_url', value: url, updated_at: new Date().toISOString() })
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <p className="text-sm text-gray-500 mb-4">แสดงในระบบและเอกสารต่าง ๆ (แนะนำ: PNG 192×192)</p>
        ...
        <div className="flex-1 pt-2">
          {logoUrl ? (
            <div className="space-y-2">
              <p className="text-sm font-medium text-green-700">✓ มีโลโก้ร้านแล้ว</p>
            ...
            <p className="text-sm text-gray-400">ยังไม่ได้อัพโหลดโลโก้<br />คลิกที่กรอบด้านซ้ายเพื่ออัพโหลด</p>
          )}
        </div>
      </div>
    </div>

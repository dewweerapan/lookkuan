              ...
              <div key={c.id} className="bg-white rounded-xl border border-gray-200 p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      {c.tier && <span className={`text-xs px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${tierConfig[c.tier as keyof typeof tierConfig]?.className}`}>{tierConfig[c.tier as keyof typeof tierConfig]?.label}</span>}
                    </div>
                    {c.phone ? (
                      <a href={`tel:${c.phone}`} className="text-sm text-brand-600 hover:underline">{c.phone}</a>
                    ) : (
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>
                    ...
                    <span className="font-semibold text-brand-600">{(c.loyalty_points ?? 0).toLocaleString()} แต้ม</span>
                  </div>
                  <div>
                ...
                <div className="mt-2 pt-2 border-t border-gray-100 flex justify-between items-center">
                  ...
                  <span className="font-bold text-brand-600">{formatCurrency(c.total_spent)}</span>
                </div>
              </div>
            ))}
          </div>

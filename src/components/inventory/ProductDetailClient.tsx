...
export default function ProductDetailClient({ product, movements }: Props) {
  const [activeTab, setActiveTab] = useState<'variants' | 'movements'>(
    'variants',
  );
              ...
              {product.variants?.map((v) => (
                <tr key={v.id}>
                  ...
                  <td className='text-right'>
                    <span
                      className={`font-bold ${v.stock_quantity <= v.low_stock_threshold ? 'text-red-600' : 'text-gray-800'}`}
                    >
                      {v.stock_quantity}
                    </span>
            ...
            <tbody>
              {movements.map((m) => (
                <tr key={m.id}>
                  ...
                  <td
                    className={`text-right font-bold ${m.quantity_change > 0 ? 'text-green-600' : 'text-red-600'}`}
                  >
                    {m.quantity_change > 0 ? '+' : ''}
                    {m.quantity_change}
                  </td>

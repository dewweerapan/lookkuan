...
export default function NewProductPage() {
  ...
  const [variants, setVariants] = useState<VariantInput[]>([
    { color: '', size: '', stock_quantity: 0, low_stock_threshold: 5, price_override: '' },
  ])
  ...
  const addVariant = () => {
    setVariants(prev => [...prev, { color: '', size: '', stock_quantity: 0, low_stock_threshold: 5, price_override: '' }])
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const variantInserts = variants
        ...
        .map(v => {
          const sku = generateSKU(product.sku_prefix, v.color || 'DEF', v.size || 'FREE')
          return {
      ...
      <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => updateForm('sku_prefix', e.target.value.toUpperCase())}
                className="pos-input"
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-lg font-bold text-gray-800">ตัวเลือกสินค้า (สี / ไซส์)</h2>
            <button
              type="button"
              onClick={addVariant}
            ...
            {variants.map((variant, index) => (
              <div key={index} className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                <div className="flex items-center justify-between mb-3">
                  <span className="font-semibold text-gray-600">ตัวเลือกที่ {index + 1}</span>
                  {variants.length > 1 && (
                    <button
                      type="button"
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                  <div>
                    <label className="text-sm font-medium text-gray-600 mb-1 block">สี</label>
                    <input
                      type="text"
                      value={variant.color}
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-600 mb-1 block">ไซส์</label>
                    <input
                      type="text"
                      value={variant.size}
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-600 mb-1 block">จำนวนสต็อก</label>
                    <input
                      type="number"
                  ...
                  <div>
                    <label className="text-sm font-medium text-gray-600 mb-1 block">ราคาพิเศษ</label>
                    <input
                      type="number"

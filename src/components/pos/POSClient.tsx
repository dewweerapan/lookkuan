    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] flex flex-col">
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-base font-bold text-gray-800">{variant.color}</span>
                  ...
                  <p className={`text-sm mt-1 ${
                    outOfStock ? 'text-red-500' : variant.stock_quantity <= 5 ? 'text-orange-500 font-semibold' : 'text-gray-400'
                  }`}>
                    {outOfStock ? 'หมดแล้ว' : `เหลือ ${variant.stock_quantity} ชิ้น`}
                  </p>
                </button>
              )
            })}
        ...
        <div className="p-4 border-t border-gray-200">
          <button onClick={onClose} className="w-full pos-btn-secondary py-3 text-lg">
            ปิด
          </button>
        </div>
      </div>
...
export default function POSClient({ categories, products }: Props) {
  ...
  const handleProductClick = (product: ProductWithVariants) => {
    ...
    if (activeVariants.length === 0) { toast.error('สินค้าหมด'); return }
    if (activeVariants.length === 1) {
      if (activeVariants[0].stock_quantity <= 0) { toast.error('สินค้าหมด'); playSound('error'); return }
  ...
  const handleVariantSelect = (variant: ProductVariant) => {
    ...
    playSound('beep')
    toast.success(`เพิ่ม ${variantPickerProduct.name} (${variant.color}/${variant.size})`)
  ...
  const handleProcessSale = async () => {
    ...
    if (paymentMethod === 'cash' && Number(cashReceived) < total) {
      toast.error('จำนวนเงินที่รับไม่พอ'); return
    }
    setProcessing(true)
    try {
      const supabase = createClient()
  ...
  const changeAmount = paymentMethod === 'cash' && cashReceived
    ? Math.max(0, Number(cashReceived) - cart.getTotal()) : 0
                ...
                return (
                  ...
                  >
                    ...
                    <div className="w-full">
                      ...
                      <p className="text-brand-600 font-bold text-lg mt-1">{formatCurrency(product.base_price)}</p>
                      <p className={`text-sm mt-1 ${totalStock <= 5 ? 'text-red-500 font-semibold' : 'text-gray-500'}`}>
                        เหลือ {totalStock} ชิ้น
                      </p>
                      {variantCount > 1 && (
                        <p className="text-xs text-blue-500 mt-0.5">{variantCount} ขนาด/สี →</p>
                      )}
                    </div>
                  </button>
                )
        ...
        `}>
                ...
                <div key={item.variant.id} className="bg-gray-50 rounded-xl p-3 border border-gray-200">
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-gray-800 text-base truncate">{item.variant.product.name}</p>
                      <p className="text-sm text-gray-500">{item.variant.color} / {item.variant.size}</p>
                      <p className="text-brand-600 font-semibold">{formatCurrency(item.unit_price)} × {item.quantity}</p>
                    </div>
                    <button
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    >−</button>
                    <span className="w-12 text-center text-xl font-bold text-gray-800">{item.quantity}</span>
                    <button
                      onClick={() => cart.updateQuantity(item.variant.id, item.quantity + 1)}
          ...
          {cart.items.length > 0 && !showPayment && (
            <div className="p-4 border-t-2 border-gray-200 bg-gray-50">
              ...
              <button onClick={() => setShowPayment(true)} className="w-full pos-btn-success text-xl py-5">
                💳 ชำระเงิน
              </button>
            </div>
          )}
          ...
          {showPayment && (
            <div className="p-4 border-t-2 border-gray-200 bg-green-50 space-y-4">
              ...
              <div className="flex gap-3">
                ...
                >
                  {processing
                    ? <span className="flex items-center gap-2"><span className="h-5 w-5 animate-spin rounded-full border-2 border-white border-r-transparent" />กำลังบันทึก...</span>
                    : '✅ ยืนยันการขาย'}
                </button>
              </div>
            </div>
